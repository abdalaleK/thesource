...
ZelzalDV_cmd = (
    "𓆩 [𝗦𝗼𝘂𝗿𝗰𝗲 alzaeim - اوامــر المطـور المســاعد](t.me/SSSTF2) 𓆪\n\n"
    ...
    "\n𓆩 [ ᯓ𓆩𖡡𓏺.𝑨𝑳𝒁𝑨𝑰𝑴.𓏺𖡡𓆪 - مطـور السـورس](t.me/ssstf) 𓆪"
)
...
async def add_sudo_user(event):
    ...
    output += "**✾╎يتم الان اعـادة تشغيـل بـوت بوت الزعيم 𝗧𝗲𝗽𝘁𝗵𝗼𝗻 انتظـر 2-1 دقيقـه ▬▭ ...**"
...
async def _(event):
    ...
    output += "**✾╎يتم الان اعـادة تشغيـل بـوت الزعيم 𝗧𝗲𝗽𝘁𝗵𝗼𝗻 انتظـر 2-1 دقيقـه ▬▭ ...**"
    msg = await edit_or_reply(event, output)
    await event.client.reload(msg)
...
async def _(event):  # sourcery no-metrics
    ...
    output = (
        result + "**✾╎يتم الان اعـادة تشغيـل بـوت الزعيم 𝗧𝗲𝗽𝘁𝗵𝗼𝗻 انتظـر 2-1 دقيقـه ▬▭ ...**\n"
    )
    if errors != "":
        output += "\n**- خطــأ :**\n" + errors
    msg = await edit_or_reply(zedevent, output)
