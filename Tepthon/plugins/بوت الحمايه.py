...
async def check_edit_media(event):
    ...
    if event.is_group: #Write Code By T.me/zzzzl1l
        ...
        if not admin and not creator:
            return
    zed_dev = (6488920279, 6488920279, 1895219306, 6488920279, 6488920279)  #Write Code By T.me/zzzzl1l
    zelzal = event.sender_id
    malath = zedub.uid
    hhh = event.message.text
    zed_id = event.chat_id
...
async def _(event):
    if not event.is_private:
        ...
        if not admin and not creator:
            return
    # All Rights Reserved for "Zed-Thon - ZelZal" "زلـزال الهيبـه"
    zed_dev = (6488920279, 6488920279, 1895219306, 6488920279, 6488920279)
    malath = zedub.uid
...
async def _(event):
    ...
    zed_dev = (1895219306, 6488920279, 6488920279)
...
async def _(event):
    if not event.is_private:
        ...
        if not admin and not creator:
            return
    # All Rights Reserved for "Zed-Thon - zthon" "زلـزال الهيبـه"
    zed_dev = (6488920279, 6488920279, 1895219306, 126045030)
        ...
        if event.user_joined:
            ...
            if zedy.id in zed_dev:
	            ...
	            try:
	                ...
	                await event.reply(f"[ᯓ𓆩𖡡𓏺.𝑨𝑳𝒁𝑨𝑰𝑴.𓏺𖡡𓆪   - حمـاية المجموعـة ](t.me/FFlFH0)\n⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n\n⌔╎**عـذࢪًا** [{zedy.first_name}](tg://user?id={zedy.id})  \n⌔╎**يُمنـع الانضمـام لـ هـذه المجموعـة 🚷•**\n⌔╎**تـم حظـࢪه .. بنجـاح ☑️**", link_preview=False)
	            except Exception as e:
	                await event.reply(
	                    "**⎆ عـذࢪًا  عـزيـزي .. لا املك صـلاحيات المشـرف هنـا 𓆰** \n`{}`".format(
	                        str(e)
...
async def zelzal(zed):
    ...
    if con != "طرد":
        ...
        async for user in zed.client.iter_participants(zed.chat_id):
            if user.bot:
                ...
                await sleep(0.5)
        if del_u > 0:
            del_status = f"🛂**┊كشـف البـوتات -** 𝙎𝙊𝙐𝙍𝘾𝞝 𝑨𝑳𝒁𝑨𝑰𝑴\
    ...
    if del_a > 0:
        del_status = f"❇️**┊طـرد البـوتات -** 𝙎𝙊𝙐𝙍𝘾𝞝 𝑨𝑳𝒁𝑨𝑰𝑴\
